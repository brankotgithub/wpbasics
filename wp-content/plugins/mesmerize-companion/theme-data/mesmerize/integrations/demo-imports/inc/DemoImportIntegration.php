...
class DemoImportIntegration {
	...
	public static function addOCDIFilters() {
		...
		if ( method_exists( OneClickDemoImport::class, 'get_instance' ) ) {
			...
			remove_action(
				'user_admin_notices',
				array( $ocdi_instance, 'start_notice_output_capturing' ),
				0
			);
			remove_action( 'admin_notices', array( $ocdi_instance, 'start_notice_output_capturing' ), 0 );
			remove_action(
				'all_admin_notices',
				array( $ocdi_instance, 'finish_notice_output_capturing' ),
				PHP_INT_MAX
			);
		...
		foreach ( $data as $key => $item ) {
			if ( isset( $item['plugins'] ) && is_array( $item['plugins'] ) ) {
				foreach ( $item['plugins'] as $plugin => $plugin_data ) {
					...
					$data[ $key ]['plugins'][ $plugin ]['activate_link'] = add_query_arg(
						array(
	...
	public static function _getDemosData() {
		$transientKey   = apply_filters(
			'extendthemes_demo_import_transient_key',
		...
		if ( ! $availableDemos ) {
			...
			$availableDemosServiceResponse = wp_remote_get(
				DemoImportIntegration::getDemosImportURL(),
				array(
					'body' => array(),
				)
			);
			...
			if ( $availableDemosServiceResponse instanceof \WP_Error ) {
				...
				if ( isset( $availableDemos['code'] ) && $availableDemos['code'] === 'extendthemes_themes_data_ok' ) {
					...
					foreach ( $availableDemos['data'] as $key => $demo ) {
						$availableDemos['data'][ $key ]['import_file_url']            = str_replace(
							'https://',
							'http://',
							$availableDemos['data'][ $key ]['import_file_url']
						);
						$availableDemos['data'][ $key ]['import_widget_file_url']     = str_replace(
							'https://',
							'http://',
							$availableDemos['data'][ $key ]['import_widget_file_url']
						);
						$availableDemos['data'][ $key ]['import_customizer_file_url'] = str_replace(
							'https://',
							'http://',
							$availableDemos['data'][ $key ]['import_customizer_file_url']
						);
					}
	...
	public static function addOCDIImportHooks() {
		if ( ! defined( 'DOING_AJAX' ) ) {
			...
			add_filter(
				DemoImportIntegration::getOption( 'info_page_tabs_filter' ),
				function ( $tabs ) {
	...
	public static function getPluginActivationURL() {
		...
		if ( ! $plugin_file || ! DemoImportIntegration::isPluginInstalled( $plugin_file ) ) {
			wp_send_json(
				array(
